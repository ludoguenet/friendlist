...
import { PhotoIcon, HeartIcon, ChatBubbleLeftIcon, ShareIcon } from '@heroicons/vue/24/outline';
...
<template>
    <main class="mx-auto max-w-2xl px-4 py-6">
      ...
      <!-- Error State -->
      <div v-else-if="error" class="rounded-xl bg-red-50 p-4 text-red-700">
          ...
          <div class="p-4 sm:p-6">
            <div class="flex items-center space-x-3">
              <div class="relative h-10 w-10 flex-shrink-0">
                <img
                  :src="feed.user.profile_picture || '/default-avatar.png'"
                  :alt="feed.user.name"
                  class="h-full w-full rounded-full object-cover ring-2 ring-white"
                />
                <div class="absolute -bottom-1 -right-1 h-4 w-4 rounded-full border-2 border-white bg-green-400"></div>
              </div>
            ...
            <div v-if="feed.attachments.length" class="mt-4">
              ...
              >
                ...
                >
                  <img
                    :src="attachment.path"
                    :alt="'Attachment in post by ' + feed.user.name"
